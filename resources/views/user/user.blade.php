...
<a href="{{ route('userAdd') }}" class="btn btn-success mb-4">Tambah User</a>
    ...
    <tbody class="text-dark" style="text-align:center">
        @foreach ($users as $user)
            <tr>
                ...
                <td class="p-4">
                    <a title="edit" class="btn m-0 p-0" target="_blank" href="{{ route('userEdit', ['id' => $user->id]) }}">
                    ...
                    <form method="POST" action="{{ route('userDelete', ['id' => $user->id]) }}" class="d-inline-block">
                        @csrf
